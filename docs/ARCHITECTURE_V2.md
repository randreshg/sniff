# Sniff 2.0 Architecture

## Executive Summary

Sniff 2.0 evolves from "environment detection library" to "development environment intelligence
platform" while preserving its core identity: zero dependencies, detection-only, frozen
dataclasses, protocol-based extensions, always succeeds.

This document defines the module organization, public API surface, extension points, backward
compatibility strategy, and migration guide for APXM (the primary consumer).
